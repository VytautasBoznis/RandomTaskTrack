...
public static class AiSystemPrompt
{
    public static string Build(DateOnly today, string timeZoneId, List<TaskDomain> domains, TaskDomain? focusDomain)
    {
        ...
        builder.AppendLine("""
            ...
            The finance tools cover accounts and their balances, recurring income and expenses, a ledger of what actually happened, stock holdings, expected dividends, deposits, debts and targets.
            ...
            - A debt pays itself too. `create_debt` amortises the schedule and the monthly payment shows up in the projection until the balance clears, so never also create a flow for it — that is the mortgage counted twice. The payment has to exceed the first month's interest or it will be refused, which is usually a sign the rate was given as a monthly figure.
            - **`paid_off_on` is the answer to "when am I free of this", not `contract_ends_on`.** They are the same until a lump sum lands and different afterwards, and the difference is the whole reason `pay_off_debt` exists. After adding one, call `project_finances` and say how much earlier it lands and how much interest it saves — do not estimate either.
            - `owed` is already subtracted from `net_worth`, and `property_assets` is already added. Report them as the parts of a total, never as adjustments still to make.
            - A debt with an `asset_value` bought something. Taking out a mortgage is not a loss the size of the mortgage, and saying so to somebody about to buy a flat is both wrong and alarming.
